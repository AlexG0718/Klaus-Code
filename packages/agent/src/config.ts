...
const ConfigSchema = z.object({
  ...
  AGENT_DB_PATH:               z.string().default(path.join(os.homedir(), '.klaus-code', 'memory.db')),
  AGENT_LOG_DIR:               z.string().default(path.join(os.homedir(), '.klaus-code', 'logs')),
  ...
  AGENT_METRICS_ENABLED:       z.string().transform((v) => v === 'true').default('true'),
  ...
  // Default: 5 minutes.
  AGENT_SESSION_CLEANUP_INTERVAL: z.coerce.number().min(60_000).default(300_000),

  // Require human approval for file-modifying patches (diff preview)
  AGENT_REQUIRE_PATCH_APPROVAL: z.string().transform((v) => v === 'true').default('false'),
  ...
  AGENT_DEBUG_MODE:            z.string().transform((v) => v === 'true').default('false'),
  ...
  DOCKER_ENABLED:              z.string().transform((v) => v !== 'false').default('true'),
...
export function loadConfig(): Config {
  ...
  if (!parsed.success) {
    console.error('❌ Config validation failed:', JSON.stringify(parsed.error.flatten().fieldErrors, null, 2));
    process.exit(1);
  }
