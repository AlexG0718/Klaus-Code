...
const MODEL_PRICING: Array<{ prefix: string; pricing: ModelPricing }> = [
  { prefix: 'claude-haiku',   pricing: { input: 0.80, output: 4.0  } },
...
export class DatabaseMemory {
  ...
  createSession(id: string, workspaceDir: string): Session {
    this.db.prepare('INSERT INTO sessions (id, workspace_dir) VALUES (?, ?)').run(id, workspaceDir);
  ...
  listSessions(limit = 20): Session[] {
    const rows = this.db
      .prepare('SELECT * FROM sessions ORDER BY pinned DESC, updated_at DESC LIMIT ?')
      .all(limit) as any[];
  ...
  updateSessionSummary(sessionId: string, summary: string): void {
    this.db
      .prepare('UPDATE sessions SET summary = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
  ...
  private rowToSession(row: any): Session {
    ...
    } catch { /* Invalid JSON, use empty array */ }
  ...
  addMessage(entry: Omit<MemoryEntry, 'createdAt'>): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      entry.id, entry.sessionId, entry.role, entry.content,
      entry.toolName ?? null, entry.toolResult ?? null,
  ...
  getKnowledge(key: string): string | undefined {
    const row = this.db.prepare('SELECT value FROM knowledge WHERE key = ?').get(key) as any;
    return row?.value;
  }

  listKnowledge(category?: string): KnowledgeEntry[] {
    const rows = (category
      ? this.db.prepare('SELECT * FROM knowledge WHERE category = ? ORDER BY key').all(category)
      : this.db.prepare('SELECT * FROM knowledge ORDER BY key').all()) as any[];
    return rows.map((row) => ({
      key: row.key, value: row.value, category: row.category, updatedAt: new Date(row.updated_at),
    }));
  }
  ...
  recordToolCall(params: {
    ...
    input: string; output?: string; success: boolean; durationMs?: number;
  }): void {
    this.db.prepare(`
      INSERT INTO tool_calls (id, session_id, tool_name, input, output, success, duration_ms)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      params.id, params.sessionId, params.toolName,
      params.input, params.output ?? null,
      params.success ? 1 : 0, params.durationMs ?? null
    );
  }

  getToolCallStats(sessionId?: string): Record<string, { calls: number; successes: number; avgDuration: number }> {
    ...
    const stats: Record<string, { calls: number; successes: number; avgDuration: number }> = {};
  ...
  recordTokenUsage(sessionId: string, inputTokens: number, outputTokens: number, model: string): void {
    ...
    logger.debug('Token usage recorded', { sessionId, inputTokens, outputTokens, model });
  }
  ...
  getFileChanges(sessionId?: string): FileChangeRecord[] {
    const FILE_MUTATING_TOOLS = [
      'write_file', 'apply_patch', 'delete_file', 'git_checkpoint',
    ];
  ...
  deleteKnowledge(key: string): boolean {
    const result = this.db.prepare('DELETE FROM knowledge WHERE key = ?').run(key);
  ...
  clearKnowledge(category?: string): number {
    const result = category
      ? this.db.prepare('DELETE FROM knowledge WHERE category = ?').run(category)
      : this.db.prepare('DELETE FROM knowledge').run();
    logger.info('Cleared knowledge', { category: category ?? 'all', deleted: result.changes });
    return result.changes;
  }
