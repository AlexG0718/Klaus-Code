...
export const MODEL_INFO: Record<ModelOption, { name: string; description: string; costMultiplier: number }> = {
  'claude-opus-4-5': { name: 'Opus', description: 'Most capable, highest cost', costMultiplier: 1 },
  'claude-sonnet-4-5': { name: 'Sonnet', description: 'Balanced performance/cost', costMultiplier: 0.2 },
  'claude-haiku-4-5': { name: 'Haiku', description: 'Fast and affordable', costMultiplier: 0.05 },
};
...
export function useAgentSocket(): UseAgentSocketReturn {
  ...
  useEffect(() => {
    ...
    socket.on('agent_event', (event: AgentEvent) => {
      handlersRef.current.forEach((h) => { try { h(event); } catch {} });
  ...
  const onEvent = useCallback((handler: (event: AgentEvent) => void) => {
    ...
    if (handlersRef.current.size >= MAX_HANDLERS) {
      console.warn(`Event handler limit (${MAX_HANDLERS}) reached. Possible memory leak.`);
  ...
  const sendPrompt = useCallback((message: string, sessionId?: string): string => {
    const sid = sessionId ?? uuidv4();
    setCurrentSessionId(sid);
    setIsRunning(true);
    socketRef.current?.emit('join_session', sid);
    socketRef.current?.emit('prompt', { message, sessionId: sid, model: selectedModel });
    return sid;
  }, [selectedModel]);
  ...
  const respondToPatchApproval = useCallback((approved: boolean, patchId: string) => {
